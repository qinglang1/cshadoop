...
@Description(
        name = "parseError",
        value = "this is a parseError function",
        extended = "eg: select parseError(line) "
)
public class ParseErrorUDTF extends GenericUDTF {

    //获取日志表所有关键字
    List<String> baseWords = MysqlUtil.getKeywords("appBaseLog");
    List<String> errorWords = MysqlUtil.getKeywords("appErrorLogs");
    ...
    public void process(Object[] args) throws HiveException {
        ...
        int errorSize = errorWords.size();

        Object[] objs = new Object[baseSize+errorSize];
        ...
        if(arr.length == 5 && arr[4] != null){
            ...
            List<String> errorLogs = JsonUtil.parseJsonToArray(arr[4], "appErrorLogs");
            for (String errorLog : errorLogs) {
                for(int i = 0; i< errorWords.size(); i++){
                    objs[i+11] = JsonUtil.parseJson(errorLog, errorWords.get(i));
                }
                forward(objs);
            }
        }
