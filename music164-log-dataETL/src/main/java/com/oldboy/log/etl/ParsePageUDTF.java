...
@Description(
        name = "parsePage",
        value = "this is a parsePage function",
        extended = "eg: select parsePage(line)"
)
public class ParsePageUDTF extends GenericUDTF {

    //获取日志表所有关键字
    List<String> baseWords = MysqlUtil.getKeywords("appBaseLog");
    List<String> pageWords = MysqlUtil.getKeywords("appPageLogs");
    ...
    public void process(Object[] args) throws HiveException {
        ...
        int pageSize = pageWords.size();

        Object[] objs = new Object[baseSize+pageSize];
        ...
        if(arr.length == 5 && arr[4] != null){
            ...
            List<String> pageLogs = JsonUtil.parseJsonToArray(arr[4], "appPageLogs");
            for (String pageLog : pageLogs) {
                for(int i = 0; i< pageWords.size(); i++){
                    objs[i+11] = JsonUtil.parseJson(pageLog, pageWords.get(i));
                }
                forward(objs);
            }
        }
